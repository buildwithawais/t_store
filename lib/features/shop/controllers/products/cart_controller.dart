...
class CartController extends GetxController {
  ...
  final variationController = AttributeController.instance;

  @override
  void onInit() {
    super.onInit();
    loadCartItems(); // Now storage should be ready
  }
  ...
  void addToCart(ProductModel product) {
    ...
    // Convert the Product Model to a CartItemModel with Given Quantity
    final selectedCartItem = convertToItem(
      product,
    ...
    if (index >= 0) {
      // This quantity is already added, Updated or Removed from the design (Cart)(-)
  ...
  void removeOneFromCart(CartItemModel item) {
    ...
    if (index >= 0) {
      if (cartItems[index].quantity > 1) {
        ...
        // Show Dialog before completely removing item from cart
        cartItems[index].quantity == 1
  ...
  void updateAlreadyAddedProductCount(ProductModel product) {
    // if the product has no variations then calculate cartEntries and display total number.
  ...
  CartItemModel convertToItem(ProductModel product, int quantity) {
    if (product.productType == ProductType.single.toString()) {
